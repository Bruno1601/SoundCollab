            ...
            <div class="p-6 border-b-8 border-gray-100 text-gray-900 dark:text-gray-100 md:flex md:justify-between md:items-center">
                <div class="space-y-3">
                    ...
                    <div class="text-sm text-gray-600">
                        ...
                        @foreach ($proyecto->users as $colaborador)
                            {{ $colaborador->name }},
                        @endforeach
                    </div>
                </div>
                ...
                <div class="flex flex-col md:flex-row items-stretch gap-3  mt-5 md:mt-0">
                    ...
                    @can('update', $proyecto)
                    <a href="{{ route('proyectos.edit', $proyecto->id) }}" class="bg-indigo-600 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center"><i class="fas fa-edit"></i></a>
                    <button wire:click="$emit('mostrarAlerta', {{ $proyecto->id }})" class="bg-red-600 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center"><i class="fas fa-trash-alt"></i></button>
                    @endcan
                </div>
            </div>
        @empty
